...
def _compose_command(spec: dict, command: list[str]) -> list[str]:
    ...
    # code overlay：纯代码发布时叠加生成的 override（只对 core 项目生效，见 cmd_code）
    overlay = _code_overlay_compose_file(spec)
    if overlay:
...
# 触发自动检测的部署目标（agent-node 也被 overlay 覆盖）
_CODE_OVERLAY_TRIGGER_SERVICES = _CODE_OVERLAY_SERVICES + ("agent-node",)
...
# 各镜像的 Dockerfile（COPY 集合的**唯一真相源**——挂载映射从导出树里的这些文件现场解析，
# 不再手抄常量；见 _parse_dockerfile_copy_map / _write_code_overlay_manifest）
_CODE_OVERLAY_DOCKERFILES = {
    "backend": "deploy/production/Dockerfile.backend",
    "agent_node": "deploy/production/Dockerfile.agent-node",
    "agent_runtime": "deploy/production/Dockerfile.agent-runtime",
    "faas_runtime": "deploy/production/Dockerfile.faas-runtime",
}
_CODE_OVERLAY_MANIFEST_NAME = ".overlay-manifest.json"


def _parse_dockerfile_copy_map(dockerfile: Path) -> list[tuple[str, str]]:
    """解析 Dockerfile 的 COPY 行 → [(仓库相对 src, 容器内绝对 dst)]。
    覆盖本仓库用到的全部形态：dir/→dir/、file→file（含重命名）、多 src→dir/。忽略 --flags。"""
    pairs: list[tuple[str, str]] = []
    for raw in dockerfile.read_text(encoding="utf-8").split("\n"):
        line = raw.strip()
        ...
        *srcs, dst = parts
        for src in srcs:
            s = src.rstrip("/")
            if src.endswith("/") or len(srcs) == 1 and not dst.endswith("/"):
                d = dst.rstrip("/")
            else:  # file → 目录 dst：basename 落入目录
                d = dst.rstrip("/") + "/" + s.split("/")[-1]
            pairs.append((s, d))
    return pairs


def _write_code_overlay_manifest(export_dir: Path, short_sha: str, version: str) -> dict:
    """从导出树自身的 Dockerfile 生成各镜像挂载映射并写入 manifest（导出完整性哨兵，
    也是 spawner 给子容器挂载时读取的唯一来源）。解析不出任何映射 → 响亮失败。"""
    images: dict[str, list[list[str]]] = {}
    for image_key, rel in _CODE_OVERLAY_DOCKERFILES.items():
        df = export_dir / rel
        if not df.is_file():
            raise RuntimeError(f"导出树缺 {rel}，无法生成 overlay 挂载映射")
        pairs = _parse_dockerfile_copy_map(df)
        if not pairs:
            raise RuntimeError(f"{rel} 未解析出任何 COPY 行")
        images[image_key] = [[src, dst] for src, dst in pairs]
    manifest = {"sha": short_sha, "version": version, "images": images}
    (export_dir / _CODE_OVERLAY_MANIFEST_NAME).write_text(
        json.dumps(manifest, ensure_ascii=False, indent=1) + "\n", encoding="utf-8")
    return manifest


def _code_overlay_cfg() -> dict:
    return dict(_cfg().get("code_overlay") or {})


def _code_overlay_compose_file(spec: dict) -> Path | None:
    """spec 属于 core compose 项目且 overlay 启用时，返回生成的 override 文件路径。"""
    if "docker-compose.core.yml" not in (spec.get("compose_files") or []):
        return None
    cfg = _code_overlay_cfg()
    if not cfg.get("enabled"):
        return None
    path = Path(str(cfg.get("compose_file") or ""))
    return path if path.is_file() else None


def _code_overlay_dir_root() -> Path:
    return Path(str(_cfg().get("paths", {}).get("data_root") or "/mnt/myapp")) / "code"


def _git_src(*args: str, check: bool = True) -> str:
    r = _run(["git", "-C", str(_source_dir()), *args], capture=True)
    if check and r.returncode != 0:
        raise RuntimeError(f"git {' '.join(args)} failed: {(r.stderr or '').strip()[:200]}")
...
def _render_code_overlay_compose(export_dir: Path, short_sha: str, version: str, manifest: dict) -> Path:
    def block(image_key: str) -> str:
        mounts = manifest["images"][image_key]
        vols = "\n".join(f"      - {export_dir}/{src}:{dst}:ro" for src, dst in mounts)
        return (
            f"    volumes:\n{vols}\n"
            f"    environment:\n"
    ...
    services = [f"  {svc}:\n{block('backend')}" for svc in _CODE_OVERLAY_SERVICES]
...
def _code_overlay_apply(names: tuple[str, ...] = _CODE_OVERLAY_SERVICES + ("agent-node",)) -> int:
    ...
    for name in names:
        ...
        if not spec:
            continue
        if _compose_cmd(spec, "deploy") != 0:
            rc = 1
    # 重建会丢手动挂接的 supabase 内网（服务端 Supabase 调用走内网 kong 依赖它）——补挂
    _ensure_supabase_internal_net(list(names), dry_run=False)
    return rc


def _code_overlay_export(ref_sha: str, version: str) -> tuple[Path, dict]:
    """全仓库 git archive 到 /mnt/myapp/code/<short>/。crash-atomic：先导出到临时目录、
    manifest（完整性哨兵）写好后整体 rename；无 manifest 的残缺目录视为脏数据重做。
    复用时 utime 刷新 mtime（prune 的 LRU 依据激活时间而非创建时间）。"""
    short = ref_sha[:12]
    root = _code_overlay_dir_root()
    export_dir = root / short
    if export_dir.is_dir():
        mf = export_dir / _CODE_OVERLAY_MANIFEST_NAME
        if mf.is_file():
            os.utime(export_dir)
            return export_dir, json.loads(mf.read_text(encoding="utf-8"))
        shutil.rmtree(export_dir, ignore_errors=True)  # 残缺导出（crash 遗留）重做
    tmp = root / f".tmp-{short}-{os.getpid()}"
    shutil.rmtree(tmp, ignore_errors=True)
    tmp.mkdir(parents=True, exist_ok=True)
    try:
        tar = _run(["sh", "-c",
                    f"git -C {shlex.quote(str(_source_dir()))} archive {shlex.quote(ref_sha)}"
                    f" | tar -x -C {shlex.quote(str(tmp))}"], capture=True)
        if tar.returncode != 0:
            raise RuntimeError(f"代码导出失败: {(tar.stderr or '').strip()[:200]}")
        manifest = _write_code_overlay_manifest(tmp, short, version)
...
def _prune_code_exports(active_dir: Path) -> None:
    """保留最近 3 份导出；**绝不**删当前激活目录（活容器 bind 在其上）。"""
    root = _code_overlay_dir_root()
    if not root.is_dir():
        return
    exports = sorted((p for p in root.iterdir() if p.is_dir() and not p.name.startswith(".tmp-")),
                     key=lambda p: p.stat().st_mtime, reverse=True)
    for old in exports[3:]:
        if old.resolve() == active_dir.resolve():
            continue
...
def _enable_code_overlay(ref: str, ref_sha: str, *, apply: bool = True, auto: bool = False) -> int:
    ...
    export_dir, manifest = _code_overlay_export(ref_sha, version)
    compose_file = _render_code_overlay_compose(export_dir, short, version, manifest)
    ...
    _prune_code_exports(export_dir)  # 两条路径（手动/auto）都清理；激活目录绝不删
    if not apply:
        return 0
...
def _code_overlay_release_plan() -> dict:
    """逐镜像发布形态：{gate_name: {tag, tag_sha, baked, retagged}}。
    retagged=True（烘焙≠tag，代码需 overlay 补齐）/ False（rebuild 型，镜像自带正确代码）
    / None（tag 非 {ver}-{sha} 形态或读不到烘焙 commit，无从判定）。"""
    plan: dict[str, dict] = {}
    for image_key, gate_name in _CODE_OVERLAY_IMAGE_KEYS.items():
        tag = str(_cfg().get("images", {}).get(image_key) or "")
        m = re.search(r":[0-9][^-]*-([0-9a-f]{7,40})$", tag)
        tag_sha = m.group(1) if m else ""
        baked = _image_baked_commit(image_key)
        retagged: bool | None = None
        if tag_sha and baked:
            retagged = not (baked.startswith(tag_sha) or tag_sha.startswith(baked[:len(tag_sha)]))
        plan[gate_name] = {"image_key": image_key, "tag": tag, "tag_sha": tag_sha,
                           "baked": baked, "retagged": retagged}
    return plan


def _maybe_auto_code_overlay(names: list[str], *, dry_run: bool) -> None:
    """deploy 时**逐镜像**识别 retag 发布（发布判定本就 per-image，混合发布是常态：
    如 trailer rebuild=backend 时 backend 重建、其余 retag）。任一镜像 retag → 启用
    overlay（否则该镜像族静默跑旧代码顶新版本号）；**全部**镜像确认 rebuild 型且此前
    的 overlay 是自动启用的 → 自动退场（回滚正确性）。非阻断。"""
    if not any(name in names for name in _CODE_OVERLAY_TRIGGER_SERVICES):
        return
    plan = _code_overlay_release_plan()
    tag_shas = {p["tag_sha"] for p in plan.values() if p["tag_sha"]}
    if not tag_shas:
        return  # 全部非不可变 tag（如 :edge），无从判定
    target = plan.get("backend", {}).get("tag_sha") or sorted(tag_shas)[0]
    if len(tag_shas) > 1:
        print(f"⚠️ 各镜像 pin 的 tag sha 不一致（{sorted(tag_shas)}），overlay 以 {target} 为准", file=sys.stderr)
    retagged = sorted(g for g, p in plan.items() if p["retagged"] is True)
    all_rebuilt = all(p["retagged"] is False for p in plan.values() if p["tag_sha"]) and not retagged
    if not retagged:
        if not all_rebuilt:
            return  # 有镜像判定不明（本地无镜像等）：不动现状，也不贸然退场
        # 全 rebuild 型：过期的 **auto** overlay 自动退场；手动 code set 的只警告
        oc = _code_overlay_cfg()
        if oc.get("enabled") and not str(oc.get("sha") or "").startswith(target[:12]):
            if oc.get("auto"):
                if dry_run:
                    print(f"+ [dry-run] 全镜像 rebuild 型发布：会自动退掉过期的 auto overlay（{oc.get('sha')}）")
                    return
                cfg = _cfg()
                cfg["code_overlay"] = {"enabled": False}
                _save_json(CONFIG_PATH, cfg, mode=0o644)
                print(f"+ 全镜像 rebuild 型发布：自动退掉过期的 auto overlay（{oc.get('sha')}）")
            else:
                print(f"⚠️ overlay 为手动设置（{oc.get('sha')}）且与本次镜像不一致——保留不动；如需回到镜像代码请 myapp-ctl code off", file=sys.stderr)
        return
    oc = _code_overlay_cfg()
    if oc.get("enabled") and str(oc.get("sha") or "").startswith(target[:12]):
        return  # overlay 已指向该发布
    if dry_run:
        print(f"+ [dry-run] retag 发布检测（{', '.join(retagged)}）→ 会自动启用 code overlay @ {target}")
        return
    print(f"+ retag 发布检测：{', '.join(retagged)} 烘焙 ≠ tag {target} → 自动启用 code overlay")
    try:
        _git_src("fetch", "--tags", "origin", check=False)
        ref_sha = _git_src("rev-parse", target)
        ...
        if reasons:
            ...
            return
        _enable_code_overlay(target, ref_sha, apply=False, auto=True)  # 随后的 compose up 会带 override 重建
    except Exception as exc:
        print(f"自动 code overlay 失败（非阻断，可手动 myapp-ctl code set {target}）: {exc}", file=sys.stderr)
...
def cmd_code(args) -> int:
    ...
    if action != "set":
        print(f"unknown code command: {action}", file=sys.stderr)
        return 2
