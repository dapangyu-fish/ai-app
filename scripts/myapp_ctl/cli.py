...
def build_parser() -> argparse.ArgumentParser:
    ...
    deploy.add_argument("--no-demo", action="store_true", help=_tx("skip out-of-the-box demo provisioning (account/FaaS service groups)", zh="跳过开箱即用 demo 装配（账号/FaaS 服务组）", de="Out-of-the-box-Demo-Provisionierung ueberspringen (Konto/FaaS-Gruppen)", es="omitir aprovisionamiento demo listo para usar (cuenta/grupos FaaS)"))
    ...
    code = sub.add_parser("code", help=_tx("code-only releases: overlay a pinned git ref over the running images (no rebuild)", zh="纯代码发布：把钉死的 git ref 覆盖到运行镜像之上（免重建）", de="Nur-Code-Releases: gepinnten Git-Ref ueber laufende Images legen (ohne Rebuild)", es="releases solo-codigo: superponer un ref git fijado sobre las imagenes (sin rebuild)"), usage=_tx("myapp-ctl code <set|status|off>", zh="myapp-ctl code <set|status|off>", de="myapp-ctl code <set|status|off>", es="myapp-ctl code <set|status|off>"))
    code_sub = code.add_subparsers(dest="code_cmd")
    code_set = code_sub.add_parser("set", help=_tx("export code at <ref> and overlay it onto backend-image services", zh="导出 <ref> 代码并覆盖到 backend 镜像族服务", de="Code bei <ref> exportieren und ueber Backend-Image-Dienste legen", es="exportar codigo en <ref> y superponerlo a los servicios de imagen backend"), usage=_tx("myapp-ctl code set <ref> [--force]", zh="myapp-ctl code set <ref> [--force]", de="myapp-ctl code set <ref> [--force]", es="myapp-ctl code set <ref> [--force]"))
    code_set.add_argument("ref", help=_tx("git ref: origin/main, a tag, or a commit sha", zh="git ref：origin/main、tag 或 commit sha", de="Git-Ref: origin/main, Tag oder Commit-SHA", es="ref git: origin/main, tag o sha"))
    code_set.add_argument("--force", action="store_true", help=_tx("skip the environment-drift gate (requirements/Dockerfile changes)", zh="跳过环境漂移检查（requirements/Dockerfile 变更）", de="Umgebungsdrift-Pruefung ueberspringen", es="omitir la comprobacion de deriva de entorno"))
    code_set.set_defaults(func=cmd_code)
    code_sub.add_parser("status", help=_tx("show overlay state and running code commit", zh="查看 overlay 状态与运行中的代码 commit", de="Overlay-Status und laufenden Code-Commit anzeigen", es="mostrar estado del overlay y commit en ejecucion"), usage=_tx("myapp-ctl code status", zh="myapp-ctl code status", de="myapp-ctl code status", es="myapp-ctl code status")).set_defaults(func=cmd_code)
    code_sub.add_parser("off", help=_tx("disable the overlay and return to image-baked code", zh="关闭 overlay，回到镜像烘焙代码", de="Overlay deaktivieren, zurueck zum Image-Code", es="desactivar overlay y volver al codigo de la imagen"), usage=_tx("myapp-ctl code off", zh="myapp-ctl code off", de="myapp-ctl code off", es="myapp-ctl code off")).set_defaults(func=cmd_code)
    code.set_defaults(func=cmd_code)
    demo = sub.add_parser("demo", help=_tx("provision or inspect the out-of-the-box demo stack (account/FaaS service groups)", zh="装配或检查开箱即用 demo 全家桶（账号/FaaS 服务组）", de="Out-of-the-box-Demo-Stack bereitstellen oder pruefen (Konto/FaaS-Gruppen)", es="aprovisionar o inspeccionar el stack demo listo para usar (cuenta/grupos FaaS)"), usage=_tx("myapp-ctl demo <provision|status>", zh="myapp-ctl demo <provision|status>", de="myapp-ctl demo <provision|status>", es="myapp-ctl demo <provision|status>"))
    demo_sub = demo.add_subparsers(dest="demo_cmd")
    demo_sub.add_parser("provision", help=_tx("ensure demo account and demo FaaS service groups", zh="确保 demo 账号与 demo FaaS 服务组就绪", de="Demo-Konto und Demo-FaaS-Gruppen sicherstellen", es="asegurar cuenta demo y grupos FaaS demo"), usage=_tx("myapp-ctl demo provision", zh="myapp-ctl demo provision", de="myapp-ctl demo provision", es="myapp-ctl demo provision")).set_defaults(func=cmd_demo)
    demo_sub.add_parser("status", help=_tx("show demo stack readiness", zh="查看 demo 装配状态", de="Bereitschaft des Demo-Stacks anzeigen", es="mostrar estado del stack demo"), usage=_tx("myapp-ctl demo status", zh="myapp-ctl demo status", de="myapp-ctl demo status", es="myapp-ctl demo status")).set_defaults(func=cmd_demo)
    demo.set_defaults(func=cmd_demo)
    test_user = sub.add_parser("test-user", help=_tx("interactively create or update a test user (username/email/password/admin)", zh="交互式创建或更新测试用户（用户名/邮箱/密码/是否管理员）", de="Testbenutzer interaktiv erstellen oder aktualisieren (Benutzername/E-Mail/Passwort/Admin)", es="crear o actualizar usuario de prueba interactivamente (usuario/email/contrasena/admin)"), usage=_tx("myapp-ctl test-user", zh="myapp-ctl test-user", de="myapp-ctl test-user", es="myapp-ctl test-user"))
    test_user.set_defaults(func=cmd_test_user)
