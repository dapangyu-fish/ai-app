...
def build_parser() -> argparse.ArgumentParser:
    ...
    deploy.add_argument("--no-demo", action="store_true", help=_tx("skip out-of-the-box demo provisioning (account/templates/FaaS service groups)", zh="跳过开箱即用 demo 装配（账号/模板/FaaS 服务组）", de="Out-of-the-box-Demo-Provisionierung ueberspringen (Konto/Templates/FaaS-Gruppen)", es="omitir aprovisionamiento demo listo para usar (cuenta/plantillas/grupos FaaS)"))
    ...
    demo = sub.add_parser("demo", help=_tx("provision or inspect the out-of-the-box demo stack (account/templates/FaaS service groups)", zh="装配或检查开箱即用 demo 全家桶（账号/模板/FaaS 服务组）", de="Out-of-the-box-Demo-Stack bereitstellen oder pruefen (Konto/Templates/FaaS-Gruppen)", es="aprovisionar o inspeccionar el stack demo listo para usar (cuenta/plantillas/grupos FaaS)"), usage=_tx("myapp-ctl demo <provision|status>", zh="myapp-ctl demo <provision|status>", de="myapp-ctl demo <provision|status>", es="myapp-ctl demo <provision|status>"))
    demo_sub = demo.add_subparsers(dest="demo_cmd")
    demo_sub.add_parser("provision", help=_tx("ensure demo account, registry templates, and demo FaaS service groups", zh="确保 demo 账号、registry 模板与 demo FaaS 服务组就绪", de="Demo-Konto, Registry-Templates und Demo-FaaS-Gruppen sicherstellen", es="asegurar cuenta demo, plantillas del registry y grupos FaaS demo"), usage=_tx("myapp-ctl demo provision", zh="myapp-ctl demo provision", de="myapp-ctl demo provision", es="myapp-ctl demo provision")).set_defaults(func=cmd_demo)
