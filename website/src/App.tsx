...
const copy = {
  zh: {
    ...
    stackItems: [
      ...
      ['AI FaaS 后端', 'AI 生成的 Python/Flask 后端：bundle 校验、隔离 git push worker、同机 faasd 运行时和路由校验的调用代理。'],
      ['Registry + Assets', '分页搜索、版本约束、组件依赖、跨实例镜像和 OSS/MinIO 资源分发。'],
    ],
    complianceTitle: '审核友好的边界',
    complianceBody: 'AI 为客户端生成的是声明式 JSON 配置：只组合已编译的控件和动作，不下发 Dart、Swift、Kotlin、插件或二进制。需要后端时，生成的 FaaS 函数运行在隔离的 faasd 运行时（服务端），同样不会向客户端下发可执行代码。',
    features: [
      ['AI 原生 DSL', '为大模型生成而设计，并渲染成真正的原生 UI，而不只是手写配置。'],
      ['全栈生成', 'AI 生成 JSON 应用，需要时再生成经校验的 Python/Flask 后端，部署到隔离的 faasd 运行时。'],
  ...
  en: {
    ...
    stackItems: [
      ...
      ['AI FaaS backends', 'AI-generated Python/Flask backends: validated bundles, an isolated git push worker, a co-located faasd runtime and a route-enforced invoke proxy.'],
    ...
    complianceBody: 'AI produces declarative JSON for the client: it composes only compiled widgets and actions, never Dart, Swift, Kotlin, plugins or binaries. When an app needs a backend, the generated FaaS functions run server-side in an isolated faasd runtime, so no executable code is shipped to the client.',
    features: [
      ['AI-native DSL', 'Structured for LLM generation and rendered to real native UI, not just hand-written config.'],
      ['Full-stack generation', 'AI emits the JSON app and, when needed, a validated Python/Flask backend deployed to an isolated faasd runtime.'],
  ...
  de: {
    ...
    stackItems: [
      ...
      ['AI FaaS-Backends', 'KI-generierte Python/Flask-Backends: validierte Bundles, isolierter Git-Push-Worker, co-located faasd-Runtime und ein route-geprüfter Invoke-Proxy.'],
    ...
    complianceBody: 'Die KI erzeugt deklarative JSON-Konfiguration für den Client: nur kompilierte Widgets und Actions, kein Dart-, Swift-, Kotlin-, Plugin- oder Binärcode. Braucht eine App ein Backend, laufen die generierten FaaS-Funktionen serverseitig in einer isolierten faasd-Runtime; es wird kein ausführbarer Code an den Client ausgeliefert.',
    features: [
      ['AI-native DSL', 'Für LLM-Generierung gebaut und in echtes natives UI gerendert, nicht nur manuelle Konfiguration.'],
      ['Full-Stack-Generierung', 'Die KI erzeugt die JSON App und bei Bedarf ein validiertes Python/Flask-Backend in isolierter faasd-Runtime.'],
  ...
  es: {
    ...
    stackItems: [
      ...
      ['Backends FaaS con IA', 'Backends Python/Flask generados por IA: bundles validados, git push worker aislado, runtime faasd co-ubicado y un proxy de invocación con rutas verificadas.'],
    ...
    complianceBody: 'La IA produce configuración JSON declarativa para el cliente: solo compone widgets y acciones ya compilados, nunca Dart, Swift, Kotlin, plugins ni binarios. Cuando una app necesita backend, las funciones FaaS generadas corren en el servidor dentro de un runtime faasd aislado; nunca se envía código ejecutable al cliente.',
    features: [
      ['DSL nativa para IA', 'Pensada para LLMs y renderizada como UI nativa real, no solo configuración manual.'],
      ['Generación full-stack', 'La IA genera la app JSON y, si hace falta, un backend Python/Flask validado en un runtime faasd aislado.'],
