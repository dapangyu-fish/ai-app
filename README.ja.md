...
- **一発でフルスタック——これが差別化要因です。** ほとんどの AI アプリビルダー（v0、Lovable、Bolt など）が渡すのは、依然として自分でホスティングして保守しなければならない*web コードベース*です。MyApp はフロントエンド**と**本物の Python/Flask FaaS バックエンドを生成します——それぞれが独立した Postgres データベース、アプリごとの権限モデル、呼び出し元ごとのデータ分離を備え、その全体を即座に実行します。別個のバックエンドプロジェクトも、デプロイステップも、ストア申請も不要です。
...
- **サーバー駆動** — 固定された事前コンパイル済みのランタイム境界を通じて、UI と振る舞いのデータを配信します。[App Store コンプライアンスノート](docs/APP_STORE_COMPLIANCE.md)を参照してください。 <sub>(この説明はかなり前に書いたもので、現在 100% 最新とは限りません。ストア掲載に向けて最善を尽くします)</sub>
...
- **バッテリー同梱** — プッシュ付き IM、AI プロキシ、パッケージレジストリ、名前空間、ミラーリング、環境切り替え——すべてが配線済みです。「認証を先送りにするまた別のローコードフレームワーク」ではありません。
- **セルフホスト可能** — `myapp-ctl deploy` が、バックエンドスタック、エージェントランタイム、レジストリ、コンフィグセンター、サービスシークレットを 1 つのホストレベル CLI から管理します。

---

## なぜこのプロジェクトが存在するのか——作者の言葉
...
- App バックエンド + AI ワーカー + Registry + コンフィグセンター
