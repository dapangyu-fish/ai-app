...
> **AIで記述する → フルスタックアプリ（UI + バックエンド + データベース）→ ユーザーのスマートフォンで即座に実行。ビルドステップも、アプリストア審査も不要。**
>
> JSON-DSL をネイティブ UI + ビジネスロジックに解釈する Flutter ランタイムです。ユーザーが望むものを AI に伝えると、AI は JSON フロントエンドを生成し、**アプリが必要とする場合には、それぞれ独立した Postgres データベースを持つ本物の Python/Flask バックエンドも**生成します。そして、事前にコンパイルされた能力セットの内部で即座にレンダリングして実行します。他の AI アプリビルダーは、自分で配線してデプロイするフロントエンドコードを渡すだけですが、MyApp はスタック全体を、すでに動作する状態で提供します。
...
1 つのリポジトリに 3 つのものが含まれています:

1. **Flutter サーバー駆動型 UI エンジン**（`lib/`）— 任意の JSON-DSL 設定を実行時に本物のクロスプラットフォームアプリにレンダリングします
2. **フルスタック AI ジェネレーター**（`backend/`, `user_center/`, `config_center/`）— AI が JSON フロントエンドを生成し、**アプリが必要とする場合には対応する FaaS バックエンド + 独立した Postgres データベースも**、認証（Supabase）、IM（OpenIM）、プッシュ（APNs + FCM）、AI チャットプロキシ、パッケージレジストリ、ユーザー管理の上に生成します
3. **パッケージエコシステム**（`templates/`）— ランタイムの上にインストールできる JSON-APP の例（IM、ゲーム、ユーザープロフィール、計算機…）

**MyApp** という名前は意図的なものです。各ユーザーは、共有ランタイムの上に「自分のアプリ（my app）」を作成、インストール、運用できます。

主力ユースケース: **ユーザーがアプリを開く → AI とチャットする（生成には通常 10〜20 分かかります）→ AI が JSON-DSL を返す → アプリがそれを即座に読み込んで実行する**——クライアントにすでにコンパイルされている能力の内部で。
...
- **AI ネイティブ** — DSL は LLM フレンドリーに設計されています。同梱の AI チャットは、3 つのプラガブルなエージェントランタイム（Claude Code、Codex、OpenCode）を通じて複数のプロバイダー（DeepSeek、MiniMax、GLM / Kimi を備えた Volcengine アグリゲーター）を実行し、実際にレンダリングされるアプリを生成します——出力を実行可能に保つための生成プレイブックと、実行中のビジュアルセルフレビューパスを備えています。
...
- **クロスプラットフォーム** — 同じ JSON-DSL が iOS、Android、Web（本番テスト済み）、macOS（実験的にテスト済み）、Linux、Windows でレンダリングされます。コア機能はすべてのプラットフォームで動作し、プラットフォーム固有の機能（IM、プッシュ）は非対応プラットフォームで優雅に低下します。
...
### エンジン
- 30 以上のウィジェット（text / button / input / list / image / video / chart / map / webview / camera / qr / チャットバブル / …）
- 15 個のカスタム演算子を備えた JsonLogic 式エンジン
- 30 以上の組み込み関数（HTTP、JSON、文字列、配列、制御フロー、UI フィードバック）
- 並行ステップのための `@parallel`
- テンプレート `{{ path }}` は元の型に解決されます（文字列化されません）
- ネットワーク / ディスク / レジストリから設定をホットスワップ
- Flame 2D ゲームエンジン（スプライト、物理、Tiled マップ）
...
- **ゼロログインデモモード** — 未認証ユーザーがフローティングボールをタップ → Demo すると、本物そっくりの AI 生成が発火し、記録されたセッションを SSE 再生して、実際に実行可能なアプリ（agent-node なし、FaaS 作成なし）を得られます——フルフローを即座に味見できます
...
| 領域 | 状態 |
|---|---|
| エンジン（Dart） | 本番運用。65k LOC。実アプリを稼働中。クライアント UI は 11 言語にローカライズ済み。 |
...
| API の安定性 | DSL v3.3 — v4 までは小さな破壊的変更があり得ます。バックエンド HTTP API は安定しています。 |
...
- [ ] 60 秒のバイラルデモ動画を投下する（AI → 約 10〜20 分で JSON 設定 → アプリが即座に実行）
- [ ] 公開ホスト型の無料ティア
