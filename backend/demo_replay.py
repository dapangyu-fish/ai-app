...
DEMO_PROMPTS = [
    ...
    # 不用客户端发版）。顺序即展示顺序。GET /api/ai/demo/list 返回它。
    {
        ...
        "title": "论坛 / 贴吧类 App（全栈）",
        "prompt": "创建一个论坛类型的 App：要有用户个人页面（显示真实头像）、可以创建讨论区（类似贴吧的「吧」）、可以发帖、评论、点赞。",
    },
    {
    ...
    },
    {
        ...
        "prompt": "做一个类似贴吧的全栈论坛 App：板块分区、发主题帖、楼中楼盖楼回帖、用户昵称主页、加好友、好友间私信，要有真实后端能保存数据。",
    },
]


def demo_prompt_list() -> list:
    """服务端下发的 demo 选择目录（uuid/title/prompt）。客户端拉取它渲染选择列表，
    所以加一个 demo 只改后端（这里 + DEMO_SESSIONS + 录制文件），不用客户端发版。
    只返回有回放映射(DEMO_SESSIONS)的项，避免目录与回放漂移。"""
    return [p for p in DEMO_PROMPTS if p.get("uuid") in DEMO_SESSIONS]
