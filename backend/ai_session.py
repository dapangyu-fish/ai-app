...
def _build_faas_backend_prompt_note(*, workspace: Optional[str] = None) -> str:
    ...
    return (
        ...
        "\n- **若本次是「多用户 / 带数据库持久化 / 论坛社区社交」类应用（任意用户互相可见、要存数据、要身份），"
        "在上面那份之外，还必须再 `cat docs/faas-fullstack-app-generation-playbook.md` 通读端到端全栈方法"
        "（需求收敛→数据模型 UUID 主键→后端 myapp_db/myapp_auth→前端→不可伪造身份/真实昵称→部署→自测），"
        "范本是 `docs/examples/tieba/`（贴吧：开吧/发帖/楼中楼无限嵌套/折叠/用户主页/好友/私信/底部 tab）。**"
        "\n- **【社交类先分叉】若需求是「微信式平台级实时社交」——搜得到平台上任何人 / 要推送 / 通讯录 / "
        "跨 App 同一个人——那**不该建 FaaS**，改 `cat docs/im-app-generation-playbook.md` 读平台 IM 生成方法"
        "（范本 `docs/examples/demo-im/` + `templates/demo_im.json`，用 `lib_im` 平台 IM 能力、**完全不写后端**）。"
        "只有「论坛/社区内部、用假名、要自定义表」的轻社交才走上面那份 FaaS 全栈方法。**"
