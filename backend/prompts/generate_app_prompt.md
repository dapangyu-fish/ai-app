...
3. 当用户已经明确要求“生成/做一个/创建一个 APP”时，禁止停在设计方案、问题清单或“请确认后我再生成”。你可以先在内部完成设计取舍，然后直接生成 JSON、校验、上传并返回 `[json_app_url]...[/json_app_url]`。只有需求缺少无法合理假设的关键信息，或框架确实无法实现时，才向用户提问。
