...
class CacheManager {
  ...
  Future<void> _saveIndex() async {
    ...
    try {
      ...
      await NativeFs.writeStringAbs(
        '$_cacheDir/index.json',
        json.encode(_indexCache),
      );
    } catch (e) {
  ...
  /// 后台异步检查并更新缓存
  Future<void> _revalidateResourceAsync(
    String name,
    VersionConstraint constraint,
    String type,
  ) async {
  ...
  Future<Map<String, dynamic>?> _downloadAndCacheResource(
    String name,
    VersionConstraint constraint,
    String type, {
    ...
    try {
      final url =
          '${RegistryConfig.registryUrl}/resolve?name=$name&version=${Uri.encodeComponent(constraint.toString())}';
      final response = await _httpClient.get(url);

      if (response['error'] != null || response['data'] == null) {
        if (!background) {
          debugPrint('[CacheManager] Registry 解析失败: ${response['error']}');
        }
        return null;
      }
      ...
      final localMatch = _findBestLocalMatch(
        name,
        VersionConstraint.parse(resolvedVersionStr),
      );
      if (localMatch != null && localMatch['version'] == resolvedVersionStr) {
        if (background) {
          debugPrint(
            '[CacheManager] 后台检查完毕，当前已是最新满足约束的版本: $name@$resolvedVersionStr',
          );
          return null; // 不需要返回 config 给后台更新使用
        }
        if (_cacheDir != null) {
          final filePath = localMatch['file_path']?.toString();
          if (filePath != null && filePath.isNotEmpty) {
            final content = await NativeFs.readStringAbs(
              '$_cacheDir/$filePath',
            );
            if (content != null) {
      ...
      if (!background) {
        debugPrint('[CacheManager] 开始下载资源: $name@$resolvedVersionStr');
      }
      ...
      if (_cacheDir == null) {
        if (!background) {
          debugPrint(
            '[CacheManager] 资源下载成功（内存模式，不落盘）: $name@$resolvedVersionStr',
          );
        }
        return config;
      }
      ...
      // 保存到本地
      final relPath =
          '${type == "app" ? "apps" : "packages"}/$name/$resolvedVersionStr.json';
      final writeOk = await NativeFs.writeStringAbs(
        '$_cacheDir/$relPath',
        json.encode(config),
      );
      if (!writeOk) {
        if (!background) {
          debugPrint(
            '[CacheManager] 缓存写入失败，直接使用远端配置: $name@$resolvedVersionStr',
          );
        }
        return config;
      }
  ...
  /// 查找本地索引中满足约束的最高版本
  Map<String, dynamic>? _findBestLocalMatch(
    String name,
    VersionConstraint constraint,
  ) {
    ...
    final versionInfo = Map<String, dynamic>.from(
      versionsNode[bestVersion] as Map,
    );
