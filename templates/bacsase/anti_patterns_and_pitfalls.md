...
## 16. 横版动作/射击游戏：sprite sheet、输入、弹丸和背景比例要闭环验证

### 表现

横版动作、平台射击、run-and-gun 类游戏里，常见“能启动但明显不对”的现象：

- 主角不是一个姿态，而是一整块多帧姿态拼图。
- 摇杆左右拖动时角色只翻面或切换朝向，但没有实际位移。
- 射击按钮只能打出第一发，后续按键没有新子弹。
- 背景看起来被放大、裁切或比例不对，和角色/地面尺度不一致。

### 反面教材分析

这类问题通常不是某个单独字段写错，而是生成时没有把“资源结构 -> 实体渲染 -> 输入 -> 每帧物理 -> 生命周期”连成闭环。

1. **把 sprite sheet 当单帧图使用**

   多帧角色图通常是网格 sprite sheet。不能只看文件名里有 `player` 就直接当 `sprite` 或 `frames: 1` 使用。
   如果图片是 360x360 的 8x8 网格，单帧就是 45x45；如果误写 `frame_size: [128,128]`，渲染出来就会是一块约 3x3 的多姿态拼图。

2. **只改 facing，不等于移动**

   虚拟摇杆只负责发 `move_axis` / `move_down` 事件。角色真正移动必须在 `flame_game.frame.logic` 中把输入量写入速度，并让物理步骤消费它。
   对 `auto_update: false` 的玩家实体，仅设置 `vx` 不会自动移动；必须调用 `@platformer.step`，且 `args.map` 要指向有效地图实体。缺少 `map` 时，平台物理不会执行，表现就是只翻面不走。

3. **子弹 id 固定导致只能打一发**

   `@spawn` 的 `id` 必须唯一；框架发现同 id entity 已存在时会拒绝生成。把所有子弹都写成 `id: "bullet"` 或 `id: "bul_1"`，第一发未销毁前后续射击都会失败。

4. **背景比例和视口比例没有一起设计**

   横版动作游戏通常需要先确定游戏视口比例，再选择或缩放背景层。不要把任意 16:9 或宽图直接按竖屏高度拉满；这会导致背景巨大、裁切明显，角色和场景尺度不一致。

### 正确姿势 / 避坑指南

- 使用 sprite sheet 前，必须确认图片尺寸和网格：`frame_size = image_size / rows_or_columns`，不要猜 64/128。
- 如果不确定帧网格，优先使用 manifest 中明确的单帧/切片图；否则先用 `src` 裁一帧验证。
- `virtual_gamepad` 输入必须形成闭环：`move_axis -> vars.move_dir -> player.vx 或 @platformer.step.move -> @platformer.step(map=...) -> camera follow`。
- `@platformer.step` 对平台类角色必须带 `map`；没有地图碰撞时，改用 `auto_update: true` 的自由像素移动，并自己处理边界/重力。
- 子弹、特效、掉落物这类动态实体必须使用递增 id 或有限对象池，例如 `bullet_{{ vars.bullet_seq }}`，生成后递增计数。
- 背景先按目标视口验算：横版优先 16:9 或接近横屏的游戏区域；竖屏 App 可以把控制区放底部，但游戏画布本身不要强行拉成竖版。

### 发布前 checklist

- 角色站立/跑动/攻击至少各验证一帧，不能出现多帧拼图。
- 摇杆左/右不仅改变朝向，还要能让 `entities.player.x` 持续变化。
- 连续按两次射击时，屏幕上要能同时或连续看到两发有效子弹。
