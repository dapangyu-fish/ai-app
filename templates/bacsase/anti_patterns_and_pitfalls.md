...
| 横版动作/射击类游戏：角色上下飞、地图只有一屏、精灵图被整张压成角色 | §17 |
...
4. 角色动画帧、敌人行为、子弹生命周期。
5. 沿路线推进的障碍、敌人、道具和目标点。

如果直接把摇杆 `move_y` 加到 `player.y`，游戏就变成了自由飞行；如果地图宽度不超过一屏，就没有横版推进；如果只有一条超长平地，就没有关卡设计；如果 sprite sheet 不切帧或裁剪尺寸超过一帧，角色就会变成多姿态拼图。
