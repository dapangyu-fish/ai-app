...
| MMC3 IRQ | **渲染线直接打拍 `m3_clock_irq`**(评审确认:线批模型内 CPUCYC 静止,a12「低≥3 周期」滤波永假,不能走 ppu_read 路径),且仅当 BG/精灵图样表**半区不同**时打拍(同半区时逐点基线为 0 拍) | 相对线计数不变,分割线不变 |
| 中线 scroll 写(可见段内) | **CPU 批在 dot-257 处切两段**,incy/copyh 在两段之间执行 → 线内前 ~86 周期的 $2005/$2000 写按硬件时序落到下一线(评审实证:SMB1 的 sprite-0 后分割写就在可见段内,原设计会错一线产生撕裂条) | 与硬件一致(线粒度) |
...
- 预算(评审校正后 + 实测):原估算过于乐观(~4×)。实测 SMB3 全帧 **~83-90ms JIT**
  (对 244ms 逐点 ≈ 2.9×);分解:APU 逐周期 ~41ms(43%)、渲染 ~25ms、CPU+事件 ~28ms。
  AOT 为 2-4× 外推带(~25-45ms,22-40fps),待真机数据点。**二期 APU 事件化为承诺项**
  (frame-step 调度化为零风险首刀),目标 ~45ms JIT ≈ 40-60fps AOT。
