...
### 9.2.5 已落地:逐扫描线精灵求值 —— 真 ROM(SMB3/MMC3)实测 6.0×

用户提供真 MMC3 ROM(SMB3 汉化卡带)后,此前「无精灵测试用例不盲发」的封印解除:

- **真游戏的帧成本被精灵扫描统治**:SMB3 一帧 **1453ms(0.7fps)** vs 纯背景测试 ROM
  261ms —— 逐像素重扫 64 条 OAM + 重取 CHR 占 **~81%**。
- **改法(纯数据层)**:每条可见扫描线 dot 1 求值一次,缓存 ≤8 个在范围精灵
  (x/attr/图样字节/OAM 序号)到 `sline`;像素级只在 ≤8 条缓存上合成。所有语义保留
  (8 精灵上限、OAM 顺序优先、sprite0、前后优先级、左缘裁剪、V/H 翻转、8x16、溢出标志)。
- **顺带修正了一处时序失真**:精灵 CHR 取数改为 A12 中性(直调 `chr_addr`)——真硬件
  每线只在精灵取数阶段升一次 A12(已由 `c==260` 合成读建模),旧逐像素 `ppu_read`
  会对 MMC3 IRQ 计数器**过打拍**。MMC2 锁存、MMC5 路径不变。
- **验证**:纯背景 ROM 校验和逐位不变;SMB3 260 帧确定性输入脚本,开场/标题/菜单逐帧
  截图肉眼比对全部正确(标题动画相位差 = IRQ 修正的预期效果,新版最贴近真硬件);
  全套 190 测试通过。
- **结果:SMB3 1453 → 244ms(6.0×,0.7 → 4.1fps JIT)**;AOT 预计 ~8–16fps。
